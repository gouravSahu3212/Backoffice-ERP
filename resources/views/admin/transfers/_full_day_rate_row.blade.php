<tr class="border-b border-gray-100 hover:bg-gray-50/50 transition-colors" data-rate-id="{{ $rate->id }}">
    ...
    <td class="py-3.5 px-4 text-sm text-gray-500">
        {{ $rate->vehicleModel?->name ?? 'All Models' }}
    </td>
    <td class="py-3.5 px-4 text-sm text-gray-500">
        {{ $rate->fare_type === 'half_day' ? 'Half Day' : 'Full Day' }}
    </td>
    <td class="py-3.5 px-4 text-sm font-medium">
    ...
    <td class="py-3.5 px-4">
        <div class="flex items-center justify-end gap-3">
            {{-- Edit --}}
            <button type="button"
                class="edit-full-day-rate-btn text-gray-400 hover:text-gray-700 transition-colors"
                data-id="{{ $rate->id }}"
                data-from="{{ $rate->from_location_id }}"
                data-to="{{ $rate->to_location_id }}"
                data-vehicle="{{ $rate->vehicle_type_id }}"
                data-vehicle-model="{{ $rate->vehicle_model_id }}"
            ...
            {{-- Toggle Status --}}
            <button type="button"
                class="toggle-full-day-status-btn relative text-gray-400 hover:text-gray-700 transition-colors"
                data-id="{{ $rate->id }}"
