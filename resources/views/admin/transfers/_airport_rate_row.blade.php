<tr class="border-b border-gray-100 hover:bg-gray-50/50 transition-colors" data-rate-id="{{ $rate->id }}">
    ...
    <td class="py-3.5 px-4 text-sm text-indigo-600 font-medium">
        {{ $rate->zone->name }}
    </td>
    <td class="py-3.5 px-4 text-sm text-indigo-600 font-medium">
    ...
    <td class="py-3.5 px-4">
        <div class="flex items-center justify-end gap-3">
            {{-- Edit --}}
            <button type="button"
                class="edit-airport-rate-btn text-gray-400 hover:text-gray-700 transition-colors"
                data-id="{{ $rate->id }}"
                data-airport="{{ $rate->airport_id }}"
                data-transfer-type="{{ $rate->transfer_type }}"
                data-zone="{{ $rate->zone_id }}"
            ...
            {{-- Toggle Status --}}
            <button type="button"
                class="airport-toggle-status-btn relative text-gray-400 hover:text-gray-700 transition-colors"
                data-id="{{ $rate->id }}"
                ...
                @endif
                <svg class="toggle-spinner hidden animate-spin w-4 h-4 text-gray-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><radialGradient id="b18" cx=".66" fx=".66" cy=".3125" fy=".3125" gradientTransform="scale(1.5)"><stop offset="0" stop-color="currentColor"></stop><stop offset=".3" stop-color="currentColor" stop-opacity=".9"></stop><stop offset=".6" stop-color="currentColor" stop-opacity=".6"></stop><stop offset=".8" stop-color="currentColor" stop-opacity=".3"></stop><stop offset="1" stop-color="currentColor" stop-opacity="0"></stop></radialGradient><circle transform-origin="center" fill="none" stroke="url(#b18)" stroke-width="15" stroke-linecap="round" stroke-dasharray="200 1000" stroke-dashoffset="0" cx="100" cy="100" r="70"><animateTransform type="rotate" attributeName="transform" calcMode="spline" dur="2" values="360;0" keyTimes="0;1" keySplines="0 0 1 1" repeatCount="indefinite"></animateTransform></circle><circle transform-origin="center" fill="none" opacity=".2" stroke="currentColor" stroke-width="15" stroke-linecap="round" cx="100" cy="100" r="70"></circle></svg>
            </button>
        </div>
    </td>
</tr>
